...
def train(cfg: DictConfig, runid: str, lap: int, src: str):
    ...
    if lap > 0:
        ...
        mu_x, cov_x = fit_moments(
            ...
            rank=cfg.fit_moments.rank, # Low-rank dimension for fit_moments
            shard=True,
    ...
    print_gpu_memory("after trainset generation")
    t3b = time.time()
    ...
    print_gpu_memory("after testset generation")
...
def main(cfg: DictConfig) -> None:
    ...
    # --- Resume support ---
    # If resuming, reuse the existing run ID; otherwise generate a new one.
    # You can specify resume_runid explicitly (e.g. from a previous run that
    # you want to continue), or let it read from current_runid.txt.
    resume_from_lap = cfg_resolved.get("resume_from_lap", None)  # None = fresh start
    resume_runid = cfg_resolved.get("resume_runid", None)        # explicit run ID override
    runid_file = PATH / 'current_runid.txt'

    if resume_from_lap is not None:
        if resume_runid is not None:
            # Explicit run ID provided on the command line
            ...
            # Fall back to the most recent run ID
    ...
    # Helper: build the run path that train() will use.
    # This must match the logic inside train().
    # On a fresh run we don't know the wandb-generated name yet, so we use
    # the DummyRun naming convention: "test_run_{runid}_{runid}"
    def _checkpoint_exists_for_lap(lap_idx: int) -> bool:
        """Postcondition: True iff the checkpoint for *lap_idx* already exists."""
        # Scan all run directories for a matching checkpoint
        runs_dir = PATH / "runs"
        if not runs_dir.exists():
            return False
        for run_dir in runs_dir.iterdir():
            if run_dir.is_dir() and runid in run_dir.name:
                ckpt = run_dir / f"checkpoint_lap{lap_idx:02d}.pkl"
                if ckpt.exists():
                    return True
        return False

    # Create one job per lap, chained sequentially
    num_laps = pbs_cfg.max_jobs
    jobs = []

    for lap in range(num_laps):
        train_fn = partial(train, cfg=cfg_resolved, runid=runid, lap=lap, src=src)

        # Capture lap in a default-argument closure for the postcondition
        _post = lambda _lap=lap: _checkpoint_exists_for_lap(_lap)

        j = job(
            train_fn,
            name=f"{pbs_cfg.name}_lap{lap:02d}",
            ncpus=pbs_cfg.ncpus,
            ngpus=pbs_cfg.ngpus,
            model=pbs_cfg.model,
            mem=pbs_cfg.mem,
            walltime=pbs_cfg.walltime,
            place=pbs_cfg.place,
            queue=pbs_cfg.queue,
        )

        # Attach postcondition so dawgz can detect already-completed laps
        j.ensure(_post)

        # Each lap depends on the previous one completing successfully
        if lap > 0:
            j.after(jobs[lap - 1], status="success")
    ...
    if pbs_cfg.dry_run:
        ...
        jobs = jobs[:pbs_cfg.max_jobs]

    print(f"Submitting {len(jobs)} sequential lap jobs via PBS...")
    for i, j in enumerate(jobs):
        deps = list(j.dependencies.keys())
        dep_names = [str(d) for d in deps] if deps else ["none"]
        print(f"  Lap {i}: {j.name} (depends on: {', '.join(dep_names)})")

    # Submit to PBS — prune=True skips laps whose checkpoints already exist
    schedule(
        *jobs,
        backend="pbs",
        ...
        env=env_commands,
        prune=True,  # <-- THIS IS THE KEY CHANGE
    )
