...
class PosteriorDenoiser(nn.Module):
    ...
    def __init__(
        ...
        self.tikhonov_min_reg = tikhonov_min_reg
        self.tikhonov_base_reg = tikhonov_base_reg
    ...
    def __call__(self, xt: Array, sigma_t: Array, key: Array = None) -> Array:
        ...
        # = 1e-5  # Minimum regularization floor
        #base_reg = 1e-8  # Base regularization coefficient
        inverse_scale = self.tikhonov_base_reg / jnp.maximum(jnp.mean(sigma_t), 1e-12)  # Prevent division by zero
        reg_factor = jnp.maximum(self.tikhonov_min_reg, inverse_scale)
        
        if self.cov_x is None:
            cov_y_xt = lambda v: self.cov_y @ v + cov_t * A(*vjp(At(v))) + (reg_factor * v) * self.use_tikhonov
        else:
            cov_x_xt = cov_t + (-(cov_t**2)) * (self.cov_x + cov_t).inv
            cov_y_xt = lambda v: self.cov_y @ v + A(cov_x_xt @ At(v)) + (reg_factor * v) * self.use_tikhonov  # Use cov_x_xt here
